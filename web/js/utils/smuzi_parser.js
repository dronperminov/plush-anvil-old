...
SmuziParser.prototype.GetCategory = function(name) {
    ...
    if (name.match(/музыкальный (мега|квиз|риск)|музыкальная среда|музыка\b|танцуем и поём|русский рок|евровидение|чисто попеть/gi))
        return "музыка"

    if (name.match(/обо вс[её]м|^телеквиз(\s+лайт)?$/gi))
        return "обо всём"
